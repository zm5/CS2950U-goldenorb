...
public class OutboundVertexQueue {
  ...
  public void sendVertex(Vertex<?,?,?> v) {
    synchronized (pvo) {
      int partitionID = orbPartition.lookupPartition(v.getVertexID());
      Vertices currentPartition = pvo.partitionVertexList.get(partitionID);
      
      Integer vertexCounter;
      synchronized (pvo.partitionVertexCounter) {
        synchronized (currentPartition) {
          vertexCounter = pvo.partitionVertexCounter.get(partitionID);
          vertexCounter++;
          pvo.partitionVertexCounter.set(partitionID, vertexCounter);
          ...
          if (vertexCounter >= maxVertices) {
            ...
            orbClients.get(partitionID).sendVertices(verticesToBeSent);
            currentPartition = vs;
            pvo.partitionVertexCounter.set(partitionID, new Integer(0));
          }
          pvo.partitionVertexList.set(partitionID, currentPartition);
        }
      }
    }
  }
